...
#aumentar a memoria do R para o Java
...
name = "psimonsi" #definir o nome para especie
...
back = BIOMOD_FormatingData (resp.var=resp, #1 para presenca
                            ...
                            PA.strategy='disk',
                            PA.dist.min=20000,	#distancia minima para gerar PA's (em m)
...
options = BIOMOD_ModelingOptions(
                        MAXENT.Phillips = list(
                             #  path_to_maxent.jar = getwd(), #deixar o arquivo maxente.jar na pasta de trabalho
                               path_to_maxent.jar = "~/R/x86_64-pc-linux-gnu-library/3.6/dismo/java", #mudar o caminho para a pasta do dismo
...
modelo_maxent = BIOMOD_Modeling(back,	#ponto de PA
                         ...
                         NbRunEval=10,	#numero de replicacoes
                         DataSplit=70,	#% de dados de presenca para treino/calibracao
                         Prevalence=0.5,
                         VarImport=10, #permutacoes para estimar a importancia das variaveis
...
#criar uma tabela com os resultados de TSS:
TSS_resultados = as.data.frame(cbind(modelos_validacao["TSS","Testing.data",,,], modelos_validacao["TSS","Sensitivity",,,], modelos_validacao["TSS","Specificity",,,], modelos_validacao["TSS","Cutoff",,,] ))

#mudar o nome das colunas
colnames(TSS_resultados) = c("TSS", "Sensitivity", "Specificity", "Threshold")
head(TSS_resultados)

#salvar os resultados
write.csv(TSS_resultados, "./Resultados/psimonsi_TSS_tabela.csv")
...
#criar uma tabela com os resultados de TSS:
AUC_resultados = as.data.frame(cbind(modelos_validacao["ROC","Testing.data",,,], modelos_validacao["ROC","Sensitivity",,,], modelos_validacao["ROC","Specificity",,,], modelos_validacao["ROC","Cutoff",,,]))

#mudar o nome das colunas
colnames(AUC_resultados) = c("AUC", "Sensitivity", "Specificity", "Threshold")
head(AUC_resultados)

#salvar os resultados
write.csv(AUC_resultados, "./Resultados/psimonsi_AUC_tabela.csv")
...
#testar diferentes valores e selecionar as replicas/modelos:
AUC_resultados[which(AUC_resultados[,1] > 0.75), ]
TSS_resultados[which(AUC_resultados[,1] > 0.75), ]

#quando decidir o valor mudar o valor no comando abaixo
posicao_modelos = which(AUC_resultados[,1] > 0.75)

#selecionar o nome dos melhores modelos e do melhor modelo:
#verificar os nomes
modelo_maxent@models.computed
#nome dos melhores modelos
melhores_modelos = modelo_maxent@models.computed[posicao_modelos]
#nome do melhor modelo que no meu caso foi o numero 5
melhor_modelo = modelo_maxent@models.computed[5]
...
#calcular a importancia e criar uma tabela com os resultados:
...
write.csv(importancia_vars, "./Resultados/psimonsi_importancia_variaveis.csv")

#selecionar a importancia considerando só os melhores modelos
importa_melhores = importancia_vars[posicao_modelos, ]

#calcular a media de importancia
media = c(mean(importa_melhores[,1]), mean(importa_melhores[,2]), mean(importa_melhores[,3]), mean(importa_melhores[,4]), mean(importa_melhores[,5])) #numero de variaveis
...
write.csv(importa_melhores_media, "./Resultados/psimonsi_importancia_variaveis_melhores.csv")
...
#criar as curvas de resposta para os melhores modelos:
curvas_melhores_modelos =
  response.plot2(
  models = meus_modelos[posicao_modelos], # escolher os modelos 
  ...
  name = "./Resultados/response_curve_psimonsi_melhores_modelos", #nome do modelo
...
curvas_melhor =
  response.plot2(
    models = meus_modelos[5], # escolher os modelos 
    Data = get_formal_data(modelo_maxent, 'expl.var') , #RasterStack com as variaveis
    show.variables = get_formal_data(modelo_maxent, 'expl.var.names'),
    do.bivariate = FALSE,
    fixed.var.metric = "mean",
    save.file = "pdf", #formato do arquivo de imagem
    name = "./Resultados/response_curve_psimonsi_melhor_modelo", #nome do modelo
    ImageSize = 480, #resolucao da imagem
...
proj_present = BIOMOD_Projection(modeling.output=modelo_maxent,
                            ...
                            proj.name='current',
                            selected.models= melhores_modelos, #selecionar os modelos bons
                            compress=FALSE,
...
plot(rasters_presente[[3]])

#salvar um modelo específico
writeRaster(rasters_presente[[3]], filename="./Resultados/psimonsi_melhor_modelo.asc", format="ascii")

#fazer um modelo médio de todas as projeções
raster_medio =  calc(rasters_presente, fun=mean)
#salvar o modelo medio
writeRaster(raster_medio, filename="./Resultados/psimonsi_modelo_medio.asc", format="ascii")
...
limiares = as.data.frame(AUC_resultados[which(AUC_resultados[,1] > 0.75), ][4])
...
writeRaster(mapa_binario, filename="./Resultados/psimonsi_mapa_binario.asc", format="ascii", overwrite=TRUE)
...
#salvar o mapa final
writeRaster(raster_final, filename="./Resultados/psimonsi_mapa_final.asc", format="ascii")
...
#salvar os resultados só dos melhores modelos
write.csv(area_adequada, "./Resultados/psimonsi_area_adequada.csv")
...
#salvar os resultados só dos melhores modelos
write.csv(area_adequada_classes, "./Resultados/psimonsi_area_adequada_classes.csv")

#salvar o raster reclassificado
writeRaster(raster_classificado, filename="./Resultados/psimonsi_mapa_final_reclassificado.asc", format="ascii")
